...
import { CommandBar, Atmosphere, Typography, Card, Image, Terminal } from "@finnminn/ui";
    ...
    <div className="min-h-screen w-full bg-void flex flex-col relative overflow-hidden">
      ...
      <main className="flex-grow z-10 px-4 py-8 md:py-16 overflow-y-auto">
        <div className="max-w-6xl mx-auto flex flex-col gap-12">
          ...
          {/* Header Section */}
          <div className="text-center space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col items-center">
            ...
            <Typography.Body variant="ectoplasm" size="sm" className="font-mono">
              ROOT_ACCESS_GRANTED // SUBJECT: {user.name?.toUpperCase().replace(/\s+/g, '_')}
            </Typography.Body>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start animate-in fade-in slide-in-from-bottom-8 duration-700 delay-100">
            ...
            <div className="lg:col-span-1 flex flex-col gap-6">
              ...
              <Terminal title="BIOMETRICS" className="w-full">
                <div className="space-y-1">
                  <Typography.Body className="text-xs">&gt; STATUS: ACTIVE</Typography.Body>
                  <Typography.Body className="text-xs">&gt; ROLE: ARCHITECT</Typography.Body>
                  <Typography.Body className="text-xs">&gt; CLEARANCE: LEVEL_OMEGA</Typography.Body>
                </div>
              </Terminal>
            </div>
              ...
              <Card variant="magic" className="p-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <Typography.Code className="text-xs text-text-muted block mb-1">SYSTEM_INTEGRITY</Typography.Code>
                      <Typography.Body className="text-sm text-ectoplasm">STABLE</Typography.Body>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-8 pt-6 border-t border-overlay">
                  ...
                  <div className="bg-void/50 p-4 font-mono text-[10px] text-witchcraft/80 rounded border border-overlay/50 overflow-x-auto">
                    <pre>{JSON.stringify({
                      ...
                      exp: "PERSISTENT",
                      scope: ["identity", "void_access", "root_console"]
                    }, null, 2)}</pre>
                  </div>
                </div>
              </Card>
          ...
          <div className="text-center opacity-30 mt-8">
            <Typography.Code className="text-[10px]">
              DATA_ORIGIN: APPS/WEB // PROTOCOL: PIXEL_GRIM // CONSOLE_VERSION: 2.0.0
            </Typography.Code>
          </div>
        </div>
      </main>
