...
- [ ] T014 Create `RitualDetailPage.tsx` in `apps/pip/src/pages/RitualDetailPage.tsx` — read `ritualId` from route params, validate it matches a loaded ritual (redirect to `/tracker` if not found), call `useRitualDetail`, render ritual name as heading using `<Typography.H1>`, render `<Tabs>` with three placeholder tabs ("Activity Log", "Trend", "Streak"); show `<Skeleton>` from `@finnminn/ui` while loading (depends on T007, T013)
...
- [ ] T018 [US2] Implement `RitualTrendGraph.tsx` in `apps/pip/src/components/habits/RitualTrendGraph.tsx` — accept `logs: HabitLog[]` prop; maintain local `range: 7 | 30 | 'all'` state (default `30`); compute `TrendDataPoint[]` (count of completed entries per calendar day in range, 0 for untracked days); render time range selector buttons using `<Button>` from `@finnminn/ui`; render pixel-art CSS bar chart matching `OracleTrends` style — vertical bars in a `flex items-end` container, bar height proportional to count (`h-1/2` for count≥1, `h-[2px]` for 0), colour `bg-ectoplasm shadow-pixel-ectoplasm`; hover tooltip shows date + count via absolute-positioned div; empty state when no data in range
...
- [ ] T020 [US3] Implement `RitualStreakGraph.tsx` in `apps/pip/src/components/habits/RitualStreakGraph.tsx` — accept `logs: HabitLog[]` prop; maintain local `range: 7 | 30 | 'all'` state (default `30`); call `calculateStreakHistory(logs, ritualId)` from `streakCalculator.ts` to get `StreakDataPoint[]`; filter to selected range; render time range selector buttons using `<Button>` from `@finnminn/ui`; render SVG polyline line graph — X-axis is date, Y-axis is streak count, polyline uses `stroke-witchcraft` (or closest PixelGrim token); render data point dots (`fill-ectoplasm`) with hover tooltip showing date + streak value; empty state when no data; `glow-witchcraft` on the SVG container `<Card>`
- [ ] T021 [US3] Wire `RitualStreakGraph` into the Streak tab of `RitualDetailPage.tsx` in `apps/pip/src/pages/RitualDetailPage.tsx` — pass `logs` from `useRitualDetail` and `ritualId` from route params
